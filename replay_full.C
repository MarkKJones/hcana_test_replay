void replay_full (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
 ...
 //=:=:=:=
  ...
  // Define DEF-file+
    analyzer->SetOdefFile("DEF-files/coin_production_sidis.def");
  // Define cuts file
  analyzer->SetCutFile("DEF-files/coin_production_sidis_cuts.def");  // optional
  //analyzer->SetCutFile("DEF-files/COIN/PRODUCTION/CUTS/coin_rawdecode_cuts.def");  // optional
  // File to record accounting information for cuts
  //  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/summary_production_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  cout << " process run " << endl;
  analyzer->Process(run);
  // Create report file from template
  //analyzer->PrintReport("TEMPLATES/COIN/PRODUCTION/coin_production_peter.template",Form("REPORT_OUTPUT/replay_coin_%d_%d.report", RunNumber, MaxEvent));  // optional
